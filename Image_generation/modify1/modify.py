...
ldm_stable = StableDiffusionPipeline.from_pretrained("../epoch1-step4000", use_auth_token=MY_TOKEN, scheduler=scheduler).to(device)
try:
...
for idx,row in enumerate(csv_reader):
    ...
    for i in phase.split(' '):
        blend1 += (i,)
    for i in new_phase.split(' '):
        blend2 += (i,)
    blend_word=(blend1,blend2)
    eq_params = {"words": (new_color), "values": (6,)} # amplify attention to the word "tiger" by *2 

    controller = make_controller(prompts, True, cross_replace_steps, self_replace_steps, blend_word, eq_params)
    images, _ = run_and_display(prompts, controller, run_baseline=False, latent=x_t, uncond_embeddings=uncond_embeddings,verbose=False)
    img=Image.fromarray(images[1])
    img = img.resize((240,640))
    target_path = os.path.join('/data2/liuzhibin/STM',new_path)
    #target_path = new_path
    img.save(target_path)
    
    ori_img = cv2.imread(image_path)
    ori_img = cv2.copyMakeBorder(ori_img, 50, 300, 50, 250, cv2.BORDER_CONSTANT, value=(255,255,255))
    cv2.putText(ori_img,phase,(0,720),cv2.FONT_HERSHEY_COMPLEX, 1.0, (0, 0, 0), 1)
    gen_img = cv2.imread(target_path)
    gen_img = cv2.copyMakeBorder(gen_img, 50, 300, 50, 250, cv2.BORDER_CONSTANT, value=(255,255,255))
    cv2.putText(gen_img,new_phase,(0,720),cv2.FONT_HERSHEY_COMPLEX, 1.0, (0, 0, 0), 1)
    
    con_img = np.concatenate([ori_img,gen_img],axis=1)
    cv2.imwrite(os.path.join('/data2/liuzhibin/STM/compare_images_cuhk_all',name),con_img)
