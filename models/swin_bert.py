...
class Swin_patch16(nn.Module):
    def __init__(self):
        super(Swin_patch16, self).__init__()
        self.swin = timm.create_model('swin_base_patch4_window12_384',pretrained=True,)
    ...
    def forward(self, x):
        x = self.swin.forward_features(x)    # 得到的是第一个cls_token的 embedding 掌握了全局的信息。
        return x

class Swin_Bert(nn.Module):
    def __init__(self, args):
        super(Swin_Bert, self).__init__()
        self.model_img = Swin_patch16()
...
if __name__ == '__main__':
    ...
    img_demo = torch.zeros((2, 3, 384, 384))
